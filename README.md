...
The source archive includes a self-contained Python receiver utility, so you do not need to already own a Silent Payments receiver to test the sender. Testnet is the default. Mainnet receiver generation requires an explicit risk flag to be passed and should be used only with a small amount.
